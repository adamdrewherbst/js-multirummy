...
//if the client claims to be a player, check their claim
if($_POST['nickname'] != NULL) include 'validate_client.php';
...
$queries = array(
	...
	'players' => array('NickName', 'RummyPlayer', 'ID <> '.($playerID === NULL ? 0 : $playerID), true),
...
do{
	foreach($queries as $attr => $fields) {
		...
		//only check the state fields that matter based on whether the user is logged in/part of a game
		if($playerID == NULL and ($attr === 'game' or $attr === 'request')) continue;
		if(($playerID == NULL or $gameID == NULL or $gameID === '0') and
		...
		if(!$fields[3]) { //query returns a single value
			...
			if(count($oldVals) + count($newVals) > 0) {
				...
				$curStr = ''; $newStr = '';
				foreach($state[$attr] as $k=>$v) $curStr .= strval($k) . ',';
				foreach($data[$attr] as $arr) $newStr .= strval($arr[0]). ',';
