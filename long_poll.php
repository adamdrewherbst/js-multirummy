...
$queries = array(
	...
	//these 3 are only checked if we are actually in a game
...
do{
	foreach($queries as $attr => $fields) {
		...
		if(($gameID == NULL or $gameID === '0') and ($attr === 'owner' or $attr === 'turn' or $attr === 'winner')) continue;
		...
		if(!$fields[3]) { //query returns a single value
			...
			if($state[$attr] === NULL) $state[$attr] = array();
			...
			foreach($state[$attr] as $key => $val) {
				if(array_search($key, $vals_first, TRUE) === FALSE) $oldVals[] = array($key);
			}
			//for each new/updated one, include its attributes as well
			foreach($vals as $val) {
				$curVal = $state[$attr][$val[0]];
				if(!array_key_exists($val[0], $state[$attr])) //new game/request
					$newVals[] = $val;
				elseif(count($val) > 1) { //existing - if this query includes attributes, see if they are updated
					$updated = false;
